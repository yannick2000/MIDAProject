...
public class HomeController implements Initializable {
    ...
    void showPreccedent() {
        ...
        String sql4= "select nomLivre,maisonedition,nomType,auteur,image from livre,type where type.idType = livre.type and nomLivre not in(select nomLivre from location) and idLivre < '"+position+"' ";
        byte byteImg[];
        Blob blob;
        try {
    ...
    public void showLivres(){
        String sql = "select count(*) from livre where nomLivre not in(select nomLivre from location )";
         int i=0;
        try {
        ...
        String sql2= "select nomLivre,maisonedition,nomType,auteur,image from livre,type where  type.idType = livre.type and nomLivre not in(select nomLivre from location)";
        byte byteImg[];
        Blob blob;
        try {
